...
function Validator() {
  ...
  const ValidatePropertyDataType = (idsProperty: IdsPropertyClass, bsddProperty: ClassPropertyContractV1) : IdsNotification | undefined =>{
            ...
            <HoverCard.Dropdown>
              {!isSubLevel && color !== "red" &&
                <Fragment key={GetUniqueKey("fragment")}>
                  <Text>{t('idsComponent.validator.matchFound')}: {name}</Text>
                  <Divider my="sm" />
                </Fragment>
              }
