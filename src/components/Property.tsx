...
function Property(props: Props) {
  ...
  useEffect(() => {
    switch (ifcProperty.type) {
      case 'IfcPropertySingleValue': {
        setInput(
          <Form.Control
            placeholder={ifcProperty.nominalValue}
      ...
      case 'IfcPropertyEnumeratedValue': {
        setInput(
          ...
          >
            {Children.toArray(
              ifcProperty.enumerationReference.enumerationValues.map((value: any, index: any) => (
      ...
      default: {
        setInput(<Form.Control placeholder={ifcProperty.name} value='{ifcProperty.nominalValue}' />)
        break
      }
    }
  ...
  return (
    <Form.Group className='mb-3 row' key={props.propertyIndex}>
      <Form.Label className='col-sm-5 col-form-label'>{ifcProperty.name}</Form.Label>
